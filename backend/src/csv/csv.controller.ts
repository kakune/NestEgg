...
import { CsvService, AuthContext, FieldMapping, ImportPreview, ImportResult, ExportOptions } from './csv.service';
...
export class CsvController {
  ...
  async uploadTransactionFile(@UploadedFile() file: Express.Multer.File) {
    if (!file) {
  ...
  async previewTransactionImport(
    ...
    return this.csvService.previewTransactionImport(dto.csvData, dto.fieldMapping, authContext);
  }
  ...
  async exportTransactions(
    ...
    const filters: TransactionFilters = {
      ...
      actorIds: query.actorIds ? (Array.isArray(query.actorIds) ? query.actorIds : [query.actorIds]) : undefined,
  ...
  @Post('incomes/upload')
  @UseInterceptors(FileInterceptor('file'))
  async uploadIncomeFile(@UploadedFile() file: Express.Multer.File) {
    if (!file) {
      throw new BadRequestException('No file uploaded');
    }
  ...
  async getTransactionTemplate(@Res() res: Response): Promise<void> {
...
2000,income,"Salary",2024-01-31,cat-salary-id,actor-user1-id,"salary","Monthly salary",false`;
  ...
  async getIncomeTemplate(@Res() res: Response): Promise<void> {
